...
module.exports = {
	...
	parseCssString: function(contents, options) {
		...
		// Process supports
		var processSupports = function(sup) {
			var strCss = '',
				query = sup.rule;

			if (!query && sup.supports) {
				query = sup.supports;
			}

			if (query) {
				strCss += '@supports ' + query + ' {';
				sup.rules.forEach(function(rule) {
					strCss += commentOrRule(rule);
				});
				strCss += '}';
			}
		...
		var outputSupports = function(sup) {
			sup.forEach(function(sup) {
				strStyles += processSupports(sup);
			});
		};

		// Check if supports was processed
